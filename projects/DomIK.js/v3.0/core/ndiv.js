...
export function autoWH( el ) { return setStyles( el, { 'width': null, 'height': null } ); }
export function fullWH( el ) { return setStyles( el, { 'width': '100%', 'height': '100%' } ); }

export function doVisual( el, rule ) { el.style['display']= rule ? 'flex':'none'; }
export function doHide( el ) { doVisual( el, false ); }
export function doShow( el ) { doVisual( el, true ); }

export function setHeight( el, value ) { setStyles( el, { 'height': value, 'min-height': value, 'max-height': value } ); }
export function setWidth( el, value ) { setStyles( el, { 'width': value, 'min-width': value, 'max-width': value } ); }
...
export function nLink( el, event_type, func, msg ) {
    if( func ) el.addEventListener( event_type, ()=>{ func( msg ); } );
    return el;
}
...
export function nText( el, text, classes, styles ) {
    ...
    item.text = ( text )=>{
        ...
        if( item.text_set ) item.innerText = item.text_set[system.lang] ? item.text_set[system.lang] : item.text_set.default;
    }
...
export function nTextArea( el, text, cols='40', rows='5', classes, styles ) {
    if( text ) {
        ...
        item.text = ( text )=>{
            ...
            if( item.text_set ){
                if( item.text_set[system.lang] ) item.textContent = item.text_set[system.lang];
...
export function nInputFile( onload = ( file ) => {}, types ) {
    ...
    if( types ) {
        let accept = '';
        types.forEach( ( type ) => { accept = accept + extensions[type] +', ' } );
...
export function nFolder( parent, title,  classes, styles ) {
    ...
    const marker = nTrigger( holder, () => {
        if( marker.index ) { doHide( folder ); } else { doShow( folder ); }
    ...
    widget.do_open = () =>  { marker.set_index( 0 ); }
    widget.do_close = () => { marker.set_index( 1 ); }
    widget.do_clear = () => { folder.do_clear(); }
    ...
    widget.get_parent = ( item ) => { return folder; }
...
export function nTabs( parent, vert=false ) {
    const container = nDiv( parent, (vert ? 'row': 'column') + ' grow alt-border grow' );
    ...
    container.addTab = ( label, icon=null, classes, styles )=>{
        ...
        const holder = nDiv( header, 'row border background panel margin ' + (vert ? 'btl bbl' : 'btl btr') );
        ...
        nLink( holder, 'click', () => { container.do_select( index ); } );
...
export function updateWidgets() {
    widgets.forEach( ( widget ) => {
        // widget.updateValue();
        if( widget.style.display === 'flex' && !widget.property.enable() ) doHide( widget );
        if( widget.style.display !== 'flex' && widget.property.enable() ) doShow( widget );
    } );
}
...
export function superWidget( el, classes='widget grow', property, label ) {
    ...
    widget.setProperty = ( value ) => { widget.property.set( value ) }
    widget.getProperty = () => { return widget.property.get(); }

    widget.updateValue = () => { widget.setValue( widget.getProperty() ); }
...
const mods = {
    properties: ( widget ) => {
        ...
        Ref.properties.getList( widget.property ).forEach( ( rec ) => {
            ...
            nWidget( widget.input, property, property.label ).do_change = ( newValue, oldValue ) => { if(widget.do_change) widget.do_change( newValue, oldValue );  };
        } );
    },
    ...
    string: ( widget ) => {
        ...
        widget.input.onchange = () => { widget.changeValue( widget.input.value || '?????' ); }
    },

    path: ( widget ) => {
        const fs = nInputFile( ( file ) => { widget.changeValue( file.filePath ); } );
    ...
    number: ( widget ) => {
        ...
        widget.input.onchange = () => { widget.changeValue( parseFloat(widget.input.value) || 0 ); }
    },
    ...
    range: ( widget ) => {
        ...
        widget.input.onchange =  () => { widget.changeValue( parseFloat(widget.input.value) || 0 ); }
        widget.metric.onchange = () => { widget.changeValue( parseFloat(widget.metric.value) || 0 ); }
    },
    ...
    vector: ( widget ) => {
        ...
        function setValue() {
            const [x, y, z, w] = [ parseFloat(vals[0].value), parseFloat(vals[1].value), parseFloat(vals[2].value), parseFloat(vals[3].value) ];
            widget.changeValue( { x:x, y:y, z:z, w:w } );
        }

        widget.setValue = ( value ) => {
            [ vals[0].value, vals[1].value, vals[2].value, vals[3].value ] = [value.x, value.y, value.z, value.w];
        }
    },
    ...
    checkbox: ( widget ) => {
        ...
        widget.setValue = ( value ) => { widget.input.checked = value; }
        widget.input.addEventListener( 'change', ( event ) => { widget.changeValue( widget.input.checked? true: false ); } );
    },
...
export function nWidget( el, prop, label ) {
    ...
    if( property.enable() ) {
        ...
        const widget = superWidget( el, 'widget-input column' + (property.hook? ' hook-border': ''), property, property.type === 'properties'? null: label);
...
export function nHierlist( parent, property )
{
    ...
    widget.set_icon   = ( item ) => { return 'empty'; },
    widget.set_label  = ( item ) => { return item.toString(); },
    widget.set_list   = ( item ) => { return item.children?item.children:[]; },
    ...
    widget.helper_enable = ( item ) => { return true; }
    ...
    function setLevel( el, item, level, is_active ) {
        ...
        level.forEach( ( value, index ) => {
            if( index ) {
                ...
                if( widget.helper_enable( item ) && widget.set_helper && is_active && index===1 ) {
                    nLink( nIcon( div, widget.helper_show? 'hier_edit': 'hier_prop' ), 'click', () => { /*widget.helper_show = !widget.helper_show; widget.changeValue( item, true );*/ } );
                } else {
    ...
    widget.addEventListener( 'dragover', ( evt ) => {
        evt.preventDefault();
        if( currentElem ) { currentElem.classList.remove( 'active-border' ); }
    ...
    function setHier( el, item, level=[2], top=true ) {
        ...
        if( count>0 ) {
            if( item.is_open ) {
                nLink( nIcon( holder, 'hier_open' + ( top?'_top':'' ) ), 'click', () => { item.is_open = false; widget.changeValue( item, true ); } );
            } else {
                nLink( nIcon( holder, 'hier_close' + ( top?'_top':'' ) ), 'click', () => { item.is_open = true; widget.changeValue( item, true ); } );
            }
        } else {
        ...
        if( is_active ) {
            ...
            //widget.scrollTop = header.offsetTop;
            //header.scrollIntoView();
            if( widget.helper_enable( item ) && widget.helper_show && widget.set_helper ) { widget.set_helper( body, item ); }
        }
