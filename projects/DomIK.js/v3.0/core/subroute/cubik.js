...
export function subroute() {
  ...
  sub.registry("set_warp", (options) => {
    ...
    DOMIK.calcDomik(app.data.dome, app.data.mirror, app.data.projector);
    ...
    uniforms.tilt = { type: "f", value: app.data.options.tilt / 180.0 };
    uniforms.flexture = { type: "f", value: app.data.options.flexture };
    uniforms.seamless = { type: "f", value: app.data.options.seamless };
    ...
    uniforms.ud_sour = {
      ...
      value: new THREE.Vector3(
        app.data.transform.sour_width,
        app.data.transform.sour_height,
        app.data.transform.sour_shift
      ),
    };
    uniforms.ud_dest = {
      type: "v3",
      value: new THREE.Vector3(
        app.data.transform.dest_width,
        app.data.transform.dest_height,
        app.data.transform.dest_shift
      ),
    };
...
export function modify() {
  if (uniforms.time) {
    const s = 1 + 2.0 * (app.data.screen.zenit - app.data.screen.front);
    const x = app.data.screen.side;
    const y = app.data.screen.front;

    uniforms["time"].value = clock.getElapsedTime();
    uniforms["mouse"].value = app.data.mouse.value;
    ...
    if (root) {
      for (const view of cube_camera_views) {
        ...
        view.camera.position.set(0, app.data.options.overhead, 0);
