    ...
    switch (res.type) {
        ...
        case 'model3d':
            ret = new Promise((resolve, reject) => {
                ROUTE.send('console_log', { stag: 'loading', tag: path, msg: ' ... ' });
                let loader;
                if (res.ext === '.fbx') loader = new FBXLoader( /*new THREE.LoadingManager()*/);
                if (res.ext === '.glb') loader = new GLTFLoader();
                if (loader) {
                    loader.load(
                        getURL(res),
                        (object) => {
                            const scene = object.scene?object.scene:object;
                            ROUTE.send('console_log', { stag: 'loading', tag: path, msg: 'ok' });
                            let box3 = new THREE.Box3().setFromObject(scene);
                            let size = new THREE.Vector3();
                            box3.getSize(size);
                            scene.size = { x: size.x, y: size.y, z: size.z }
                            resolve(conv(scene, res));
                        }
                    );
                }
            });
...
export const ModelTools = {
    ...
    getResource: async (path, name = null) => {
        if (path && path !== 'empty') {
            if (!resources[path]) {
                let ret = await loadResource(path)
                resources[path] = ret;
                if (name) resources[name] = ret;
            }
