...
function select(ref, flag = true) {
  if (ref) {
    if (ref.hier) {
      ref.hier.item.style.backgroundColor = flag
        ? rgb2rgbtxt(100, 100, 155)
        : null;
    }
  }
}
...
function mkHierEditor(el) {
  ...
  hier.drag = (sour, dest) => {
    ROUTE.send("ops_ref_edit", { ref: sour, dest: dest, mode: "moveto" });
  };
  ...
  hier.set_helper = (el, item) => {
    ...
    if (ulist) {
      if (item.type === "template") {
        ...
        const panel = nDiv(
          pn,
          "column active-background margin-big border-helper"
        );
        ulist.forEach((param) => {
          const property = param.property;
          if (property.enable()) {
  ...
  hier.set_header = (header, item) => {
    ...
    mods =
      item.model && item.model.group && item.model.group.visible
        ? "visual"
        : "";
      ...
      if (1) {
        ...
        if (ref && ref.model && ref.model.group) {
          ...
          nText(
            lt,
            "    group.scale " +
              frmt(ref.model.group.scale.x) +
              ":" +
              frmt(ref.model.group.scale.y)
          );
          nText(lt, "_flat level " + level);
          nText(lt, "_flat level " + ilevel);
          nText(lt, "_flex:basis " + flex + ":" + reflex + ":" + basis);
          if (side)
            nText(lt, "_______side " + frmt(side.x) + ":" + frmt(side.y));
          if (rect)
            nText(lt, "_______rect " + frmt(rect.x) + ":" + frmt(rect.y));
          ...
          if (ref.model.shape) {
            ...
            nText(
              lt,
              "shape.scale " +
                frmt(ref.model.shape.scale.x) +
                ":" +
                frmt(ref.model.shape.scale.y)
            );
          }
        }
      }
...
function make_ui() {
  ...
  for (let type of getListRefObjects()) {
    ...
    ops.create[name] = {
      ...
      run: () => {
        ...
        const options = {
          ref: nref,
          from: data.active.get()
            ? data.active.get()
            : app.project.presentation,
        };
        ROUTE.send("create_ref", {
          ref: nref,
          from: data.active.get()
            ? data.active.get()
            : app.project.presentation,
  ...
  ops.edit = {
    moveup: {
      ...
      icon: "arrow-up",
      run: () => {
        ROUTE.send("ops_ref_edit", { ref: data.active.get(), mode: "move-up" });
      },
    },
    movedown: {
      info: "Move down",
    ...
    copy: {
      ...
      icon: "copy",
      run: () => {
        ROUTE.send("ops_ref_edit", { ref: data.active.get(), mode: "copy" });
      },
    },
    paste: {
      info: "Paste",
      icon: "paste",
      run: () => {
        ROUTE.send("ops_ref_edit", { ref: data.active.get(), mode: "paste" });
      },
    },
    cut: {
      info: "Cut",
      icon: "cut",
      run: () => {
        ROUTE.send("ops_ref_edit", { ref: data.active.get(), mode: "cut" });
      },
    },
    delete: {
      info: "Delete",
      icon: "delete",
      run: () => {
        ROUTE.send("ops_ref_edit", { ref: data.active.get(), mode: "delete" });
      },
    },
  };
  ...
  ops.file = {
    ...
    download: {
      ...
      icon: "download",
      run: () => {
        ROUTE.send("ops_db", { mode: "download" });
      },
    },
    upload: {
      info: "Upload",
...
function mkViewport(el) {
  ...
  controls.target = new THREE.Vector3(0, app.data.options.overhead, 0);
  ...
  const dome = new THREE.LineSegments(
    edome,
    new THREE.LineBasicMaterial({ color: theme.colorGrid })
  );
  ...
  const overhead = new THREE.Mesh(
    new THREE.SphereGeometry(radius * 0.02, divisions, divisions),
    new THREE.MeshBasicMaterial({ color: theme.colorHead })
  );
  orbitHelper.add(overhead);
  scene.add(orbitHelper);

  const viewHelper = new THREE.Group();
  const viewGrid = new THREE.GridHelper(
    divisions,
    divisions,
    theme.colorBase,
    theme.colorGrid
  ...
  sub.registry("set_warp", (options) => {
    overhead.position.set(0, app.data.options.overhead, 0);
    controls.target = new THREE.Vector3(0, app.data.options.overhead, 0);
  });
...
function mkModelEditor() {
  ...
  const workarea = nDiv(null, "column grow border");
  const header = nDiv(workarea, "row");

  sub.registry("do_view", (view) => {
    setParent(workarea, view.canvas);
    view.workarea = workarea;
  });

  const vp = mkViewport(workarea);
  const vw = mkViewwarp(workarea);

  function set_mode(dtype) {
    vp.viewport_mode = dtype;
  ...
  const p_start = nButton(
    upanel,
    "",
    "animation_start:mrl mrr",
    () => {
      ROUTE.send("animation", { cmd: "start", ref: app.project.presentation });
    },
    "btl bbl bbr btr alt-border background"
  );
  const ppanel = nDiv(upanel, "row");
  const p_stop = nButton(
    ppanel,
    "",
    "animation_stop:mrl",
    () => {
  ...
  const p_resume = nButton(
    ppanel,
    "",
    "animation_resume:mrr",
    () => {
      anim_mode("resume");
    },
    "btr bbr alt-border background"
  ...
  nButton(
    ...
    () => {
      if (app.project)
        ROUTE.send("show-mode", { ref: app.project.presentation });
    },
    "btl bbl bbr btr alt-border background"
  );
  nButton(
    ...
    () => {
      if (app.project)
        ROUTE.send("edit-mode", { ref: app.project.presentation });
    },
    "btl bbl bbr btr alt-border background"
  );
...
export function subroute() {
  ...
  sub.registry("edit-mode", () => {
    set_mode("edit-mode");
  });
  sub.registry("show-mode", () => {
    set_mode("show-mode");
  });
  ...
  sub.registry("setup", () => {
    const desktop = nDiv(document.body, "", {
      position: "absolute",
      width: "100%",
      height: "100%",
      "z-index": "200",
