...
export function send(type, options = {}) {
  if (ops_list[type]) {
    ops_list[type].ops.forEach((task) => {
      exec(task.listener[type], options);
    });
  }
}
...
export function updateLock(tag) {
  _update_lock_.push(tag);
}
export function updateUnLock(tag) {
  _update_lock_.removeOf(tag);
}
