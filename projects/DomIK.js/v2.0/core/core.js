...
function init() {
    ...
    DOMIK.init( options );
    WARP_WIRE.init( options );
    WARP_TEXTURE.init( options );
...
function gui_tools_visual( el, show ) {
    el.tabIndex="0";
    el.do_hide = ()=> { el.style['display']='none'; return el; }
    el.do_show = ()=> { el.style['display']='flex'; return el; }
    if( show ) { el.do_show() } else { el.do_hide() };
    return el;
}
...
function changeDemoset( demo, onload=null ) {
    options.tools.transformType = demo.transformType;
    options.tools.wireframe = demo.wireframe;
    
    if( options.content.data ){
        if( options.content.data=='video' ) {
...
function mkGallery( styles, onload=null ) {
    ...
    const folder = canvas.folder( 'Галлерея' );
...
function mkMediaFile( onload ) {
    ...
    el.onload = ( file )=>{
        ...
        const demo = { type: type, content: null, file: file, transformType: options.tools.transformType, wireframe: true, title: label };
        changeDemoset( demo, onload );
    }
...
function mkPlayer( styles ) {
    const canvas = mkCanvas( styles );
    const el = gui_tools_div( canvas.div( 'column' ) );
...
//    const domik = menu.button().setIcon( 'domik' );
//    const smenu = gui_tools_visual(menu.div( 'row' ), true );
    canvas.media = menu.button( 'Галлерея', 'media' );
    canvas.files = menu.button( 'Файлы', 'files' );
    canvas.tools = menu.button( 'Настройки', 'tools' );

    let s = true;
//    link( domik, 'click', ()=>{  s=!s; if( s ) { smenu.do_show(); } else { smenu.do_hide(); }; } );
    return canvas;
}
...
function mkFilesMenu( styles ) {
    ...
    canvas.equir = menu.button( 'Equirect', 'equirectangular' );
...
function mkToolsMenu( styles ) {
    ...
    folder_transform.range( options.tools, 'factor', 1.0, 2.0, 0.1 ).setLabel( 'Охват' );
...
function initDemoset() {
    options.content = {}

    const do_play = ()=>{ player.do_change(); do_hide(); update(); updateWidgets(); }

    const menu =   mkMainMenu( { 'width':'25vw', 'height':'8vh',  'margin-top':'0vh' } );
    const gallery = mkGallery( { 'width':'25vw', 'height':'92vh', 'margin-top':'8vh' }, do_play );
    const files = mkFilesMenu( { 'width':'25vw', 'height':'92vh', 'margin-top':'8vh' } );
    const tools = mkToolsMenu( { 'width':'25vw', 'height':'92vh', 'margin-top':'8vh' } );

    const do_hide = ()=> { gallery.do_hide(); files.do_hide(); tools.do_hide() };

    do_hide();

    const player = mkPlayer( { 'width':'15vw', 'height':'15vw', 'margin-left':'85vw', 'overflow':'hidden auto' } );
    ...
    link( files.cubem, 'click', ()=>{ options.toolsp.transformType='Cubemap'; mediaFile.do_select(); updateWidgets(); } );
}
